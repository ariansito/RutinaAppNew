...
document.addEventListener("DOMContentLoaded", () => {
    ...
    setTimeout(checkDataIntegrity, 2000);
});
...
function setupEventListeners() {
    ...
    if (debugBtn) {
        ...
        console.log("Debug button found and configured");
    } else {
...
// CRITICAL: These are the functions that were missing!
function saveTasks() {
    try {
        ...
        if ('serviceWorker' in navigator && navigator.serviceWorker.controller) {
            ...
            console.log('Data sync message sent to Service Worker');
        } else {
            console.log('Service Worker not available for sync');
        }
        ...
        if (savedTasks) {
            ...
            console.log('Verification: Tasks in localStorage after save:', parsedTasks.length);
        }
        ...
        // Fallback: try to save in chunks
        try {
            const chunkSize = 100;
            for (let i = 0; i < tasks.length; i += chunkSize) {
                const chunk = tasks.slice(i, i + chunkSize);
                localStorage.setItem(`rutinaApp_tasks_chunk_${i}`, JSON.stringify(chunk));
            }
            localStorage.setItem("rutinaApp_tasks_chunks", Math.ceil(tasks.length / chunkSize).toString());
            console.log('Tasks saved in chunks as fallback');
        } catch (fallbackError) {
    ...
    try {
        ...
        try {
            ...
            for (let i = 0; i < events.length; i += chunkSize) {
                ...
                localStorage.setItem(`rutinaApp_events_chunk_${i}`, JSON.stringify(chunk));
            }
            localStorage.setItem("rutinaApp_events_chunks", Math.ceil(events.length / chunkSize).toString());
            console.log('Events saved in chunks as fallback');
        } catch (fallbackError) {
            console.error('Fallback save also failed:', fallbackError);
        }
    }
...
function loadTasks() {
    ...
    // Try loading from chunks if main storage failed
    try {
        const chunksCount = localStorage.getItem("rutinaApp_tasks_chunks");
        if (chunksCount) {
            tasks = [];
            for (let i = 0; i < parseInt(chunksCount); i++) {
                const chunk = localStorage.getItem(`rutinaApp_tasks_chunk_${i}`);
                if (chunk) {
                    const parsedChunk = JSON.parse(chunk);
                    tasks = tasks.concat(parsedChunk);
                }
            }
            console.log('Tasks loaded from chunks:', tasks.length);
            // Restore to main localStorage
            saveTasks();
            return;
        }
    } catch (chunkError) {
        console.error('Error loading tasks from chunks:', chunkError);
    }
...
function loadEvents() {
    ...
    // Try loading from chunks if main storage failed
    try {
        const chunksCount = localStorage.getItem("rutinaApp_events_chunks");
        if (chunksCount) {
            events = [];
            for (let i = 0; i < parseInt(chunksCount); i++) {
                const chunk = localStorage.getItem(`rutinaApp_events_chunk_${i}`);
                if (chunk) {
                    const parsedChunk = JSON.parse(chunk);
                    events = events.concat(parsedChunk);
                }
            }
            console.log('Events loaded from chunks:', events.length);
            return;
        }
    } catch (chunkError) {
        console.error('Error loading events from chunks:', chunkError);
    }
...
function checkDataIntegrity() {
    ...
    if (!Array.isArray(tasks)) {
        console.warn('Tasks array is corrupted, resetting...');
        tasks = [];
        saveTasks();
    }
    ...
    if (!Array.isArray(events)) {
        console.warn('Events array is corrupted, resetting...');
        events = [];
        saveEvents();
    }
    
    // Validate individual task objects
    tasks = tasks.filter(task => {
        if (!task || typeof task !== 'object' || !task.id || !task.title) {
            console.warn('Invalid task found, removing:', task);
            return false;
        }
        return true;
    });
    
    // Validate individual event objects
    events = events.filter(event => {
        if (!event || typeof event !== 'object' || !event.id || !event.title) {
            console.warn('Invalid event found, removing:', event);
            return false;
        }
        return true;
    });
    ...
    if (events.length > 0) saveEvents();
    
    console.log('Data integrity check completed. Tasks:', tasks.length, 'Events:', events.length);
}
...
function debugStorage() {
    ...
    try {
        const localStorageTasks = localStorage.getItem("rutinaApp_tasks");
        const localStorageEvents = localStorage.getItem("rutinaApp_events");
        const sessionStorageTasks = sessionStorage.getItem("rutinaApp_tasks_backup");
        
        console.log('localStorage tasks:', localStorageTasks ? JSON.parse(localStorageTasks) : 'null');
        console.log('localStorage events:', localStorageEvents ? JSON.parse(localStorageEvents) : 'null');
        console.log('sessionStorage backup:', sessionStorageTasks ? JSON.parse(sessionStorageTasks) : 'null');
