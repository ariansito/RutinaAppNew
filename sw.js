...
self.addEventListener('fetch', (event) => {
  ...
  if (event.request.url.includes('.html') || event.request.url.includes('index.html')) {
...
self.addEventListener('message', (event) => {
  ...
  if (event.data && event.data.type === 'DATA_UPDATED') {
    console.log('Data updated in Service Worker:', event.data);
...
async function storeDataOffline(data) {
  try {
    const db = await openDB('RutinaAppDB', 1, {
      upgrade(db) {
